//-----(Youssef Bot by MEMO Bot Inc.)-----\\
//-----(Youtube Channel: Youssef Botrouss)-----\\
...
if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		...
		const reply = (teks) => {
			alpha.sendMessage(from, teks, text, { thumbnail: ofrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `Doge Bot`,body:"HORNY = BONK",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/p/6349482305092740/201289267791`}}})
		}
		const simir = (teks) => {
...
case 'stupid':
      ...
      if (!isGroup) return reply(mess.only.group)
        ...
        break
        //funmenu by Youssef-Bot
                        case 'uglycheck':
					// source code by Youssef-Bot⛔
...
stamtus =`❏ 「 \`\`\`MAKER MENU\`\`\` 」
────────────────────
_example: ${prefix}throne Youssef-Bot_
────────────────────
🐶 ${prefix}write 「 text 」
🐶 ${prefix}throne 「 text 」
🐶 ${prefix}logoff 「 text 」
...
break
       ...
       case 'logoa':
       if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: mek})
                   if (args.length < 1) return reply(`[  ×  ] Example :\n*${prefix}${command} Youssef-Bot&lol*`)
                   var F = body.slice(7)
				   var F1 = F.split("|")[0];
				   var F2 = F.split("|")[1]; 
                   reply(mess.wait)
